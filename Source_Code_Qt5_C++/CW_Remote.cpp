...
//Kenneths-MacBook-Pro:build-CW_Remote-Desktop_Qt_5_12_2_clang_64bit-Release Ken$ otool -L CW_Remote.app/Contents/MacOs/CW_Remote
...
//Kenneths-MacBook-Pro:build-CW_Remote-Desktop_Qt_5_12_2_clang_64bit-Release Ken$
...
Get_Metric_Statistics ( int Graph_Metric_Descriptor_Index,
    ...
    Aws::CloudWatch::Model::Dimension request_dimension;
    ...
    for (int idx = 0; idx < statistics.size(); idx++) {
        ...
        Aws::CloudWatch::Model::Statistic statistic = CloudWatch_Statistics[stat_string];
        // {@@@@@} Should check for error here.
    ...
    // {@@@@@} Should check for error here.
    Aws::CloudWatch::Model::StandardUnit unit = CloudWatch_Units[unit_string];
    // {@@@@@} Should check for error here.
    request.SetUnit(unit);

    Aws::CloudWatch::Model::GetMetricStatisticsOutcome raw_metric_statistics_outcome =
        CloudWatch_Client->GetMetricStatistics(request);

    Aws::CloudWatch::Model::GetMetricStatisticsResult raw_metric_statistics_result;

    if (raw_metric_statistics_outcome.IsSuccess())
        raw_metric_statistics_result = raw_metric_statistics_outcome.GetResult();

        Aws::Vector<Aws::CloudWatch::Model::Datapoint> raw_metric_datapoints =
