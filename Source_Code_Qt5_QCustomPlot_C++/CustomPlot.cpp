...
CustomPlot::CustomPlot( const QVector<Metric_Statistics_Descriptor> Chart_Metric_Statistics_List,
    ...
    xAxis->setRange(data_min_datetime.toSecsSinceEpoch(),
                    data_max_datetime.toSecsSinceEpoch());
...
CustomPlot::initialize_plot_private_values ( const QVector<Metric_Statistics_Descriptor> Chart_Metric_Statistics_List ) {
    ...
    data_min_datetime = datapoints[0].datapoint_datetime;
    data_max_datetime = datapoints[0].datapoint_datetime;
...
CustomPlot::plot_datasets ( const QVector<Metric_Statistics_Descriptor> Chart_Metric_Statistics_List ) {
    for (int line_idx = (Chart_Metric_Statistics_List.size() - 1); line_idx >= 0; line_idx--) {
        ...
        QList<Metric_Statistics_Datapoint> datapoints =
            Chart_Metric_Statistics_List[line_idx].Datapoints_Maximum_List;

        QVector<double> datapoint_time(datapoints.size());
        QVector<double> datapoint_value(datapoints.size());
        QVector<QCPGraphData> timeData(datapoints.size());
        for (int data_idx = 1; data_idx < datapoints.size(); data_idx++) {
            if (datapoints[data_idx].datapoint_datetime < data_min_datetime)
                data_min_datetime = datapoints[data_idx].datapoint_datetime;
            if (datapoints[data_idx].datapoint_datetime > data_max_datetime)
                data_max_datetime = datapoints[data_idx].datapoint_datetime;

            if (which_y_axis == "left") {
                if (datapoints[data_idx].datapoint_value < data_min_value_left)
                    data_min_value_left = datapoints[data_idx].datapoint_value;
                if (datapoints[data_idx].datapoint_value > data_max_value_left)
                    data_max_value_left = datapoints[data_idx].datapoint_value;
            }
            else if (which_y_axis == "right") {
                if (datapoints[data_idx].datapoint_value < data_min_value_right)
                    data_min_value_right = datapoints[data_idx].datapoint_value;
                if (datapoints[data_idx].datapoint_value > data_max_value_right)
                    data_max_value_right = datapoints[data_idx].datapoint_value;
            }

            datapoint_time[data_idx] =
                QDateTime(datapoints[data_idx].datapoint_datetime).toSecsSinceEpoch();
            datapoint_value[data_idx] = datapoints[data_idx].datapoint_value;
        }

        graph_ptr->setData(datapoint_time, datapoint_value);
    }
}
...
CustomPlot::mousePressEvent ( QMouseEvent *event ) {
    if (zoom_level == 0) {
        ...
        if ((plotarea_rect->left() < event->pos().x()) and
            ...
            xAxis->setRange(zoom_min_datetime.toSecsSinceEpoch(),
                            zoom_max_datetime.toSecsSinceEpoch());
            if (left_y_axis_series_count > 0) {
